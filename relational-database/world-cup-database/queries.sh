...
echo "$($PSQL "SELECT name FROM games LEFT JOIN teams on games.winner_id = teams.team_id WHERE year = 2014 AND round = 'Eighth-Final' UNION SELECT name FROM games LEFT JOIN teams on games.opponent_id = teams.team_id WHERE year = 2014 AND round = 'Eighth-Final' ORDER BY name ASC")"
